...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function handle($screenName, $priority, $object = null)
    {
        if ($screenName !== CUSTOM_FIELD_MODULE_SCREEN_NAME && $priority == 'advanced') {
            ...
            if (defined('CATALOG_PRODUCT_MODULE_SCREEN_NAME')) {
                switch ($screenName) {
                    case CATALOG_CATEGORY_MODULE_SCREEN_NAME:
                        add_custom_fields_rules_to_check([
                            CATALOG_CATEGORY_MODULE_SCREEN_NAME => isset($object->id) ? $object->id : null,
                            'model_name' => CATALOG_CATEGORY_MODULE_SCREEN_NAME,
                        ]);
                        break;
                    case CATALOG_PRODUCT_MODULE_SCREEN_NAME:
                        add_custom_fields_rules_to_check([
                            'model_name' => CATALOG_PRODUCT_MODULE_SCREEN_NAME,
                        ]);
                        break;
                }
            }
